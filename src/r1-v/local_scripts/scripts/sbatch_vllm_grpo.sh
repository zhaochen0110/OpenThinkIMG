...
#SBATCH --job-name=Qwen2-VL-2B-GRPO-CHARTGEMMA
#SBATCH --output=/mnt/petrelfs/songmingyang/code/reasoning/R1-V/src/r1-v/local_scripts/scripts/logs/%x-%j.log
#SBATCH --error=/mnt/petrelfs/songmingyang/code/reasoning/R1-V/src/r1-v/local_scripts/scripts/logs/%x-%j.log
...
#SBATCH -w SH-IDCA1404-10-140-54-5
...
export RUN_NAME="Qwen2-VL-2B-GRPO-CHARTGEMMA"
...
cpus=2
quotatype="reserved"
CUDA_VISIBLE_DEVICES="2,3,4,5,6" srun --partition=MoE --job-name="eval" --mpi=pmi2  --gres=gpu:${gpus} -n1 --ntasks-per-node=1 -c ${cpus} --kill-on-bad-exit=1 --quotatype=${quotatype}  \
