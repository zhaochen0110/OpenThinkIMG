...
from llava_plus.conversation import (default_conversation, conv_templates,
                                SeparatorStyle)
from llava_plus.constants import LOGDIR
from llava_plus.utils import (build_logger, server_error_msg,
                         violates_moderation, moderation_msg)
import hashlib
from llava_plus.serve.utils import annotate_xyxy, show_mask
...
def http_bot(state, model_selector, temperature, top_p, max_new_tokens, with_debug_parameter_from_state, request: gr.Request):
    ...
    if len(state.messages) == state.offset + 2:
        ...
        if "llava_plus" in model_name.lower():
