...
def load_dataset(file_path, num_samples=None):
    ...
    new_dataset = []
    for item in dataset:
        if "reachqa" not in item["image_path"]:
            new_dataset.append(item)
            
    if num_samples is None:
        return Dataset.from_dict({"data": new_dataset})
    ...
    # return Dataset.from_dict({"data": dataset[1000:1000+num_samples]+dataset[-(num_samples+1000):-1000]})
    return Dataset.from_dict({"data": new_dataset[:num_samples]})
