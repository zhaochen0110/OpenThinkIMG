...
# this code is used to evaluate the sft qwen models on Reachqa, chartqa, and charxiv
import argparse
...
def initialize_model(model_path, tensor_parallel_size, max_model_len=12800):
...
def run_inference_and_save(file_path, model_path, image_base_path, output_path, num_samples, tensor_parallel_size, batch_size=2000):
    ...
    for i in tqdm(range(0, total_samples, batch_size), desc="Processing batches"):
        ...
        for sample in batch:
            ...
            image_path = str(sample.get("figure_id")) + '.jpg'
            if not image_path:
                continue
            ...
            if "internvl" in model_path.lower():
                ...
                messages = [{'role': 'assistant', 'content': assistant_prompt_reachqa}, {'role': 'user', 'content': f"<image>\n{question}"}]
            ...
            if "llava-onevision" in model_path.lower():
                prompt = f"<|im_start|>system\n{assistant_prompt_reachqa}<|im_end|>\n \
                <|im_start|>user <image>\n{question}<|im_end|> \
                <|im_start|>assistant\n"
                stop_token_ids = None
        ...
        for idx, output in enumerate(outputs):
            ...
            image_path = batch_data[idx]["figure_id"]
            label = batch_data[idx]["answer"]
