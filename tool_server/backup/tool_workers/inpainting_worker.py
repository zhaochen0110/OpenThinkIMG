...
class ModelWorker:
    ...
    def generate_stream_func(self, model, params, device):
        ...
        mask_rle = params["mask"] # rle

        # load image and run models
        image = self.load_image(image_path)
        mask = Image.fromarray(mask_util.decode(mask_rle) * 255)

        # resize to 512, 512
        w, h = image.size
        image = self.resize(image, 512, 512)
        mask = self.resize(mask, 512, 512)

        # run
        images = self.pipe(prompt, image=image, mask_image=mask).images
        image = images[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=int, default=23336)
    parser.add_argument("--worker-address", type=str, default="http://localhost:23336")
    parser.add_argument(
